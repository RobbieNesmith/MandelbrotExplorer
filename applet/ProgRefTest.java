...
public class ProgRefTest extends PApplet {
	...
	public void setup()
	{
		...
		imag = .4f;
		// drawJulia(0.2f,0.4f);
		drawMandel(minX, minY, maxX, maxY, maxiter, p, step);
	}
	...
	{
		for (int i = step; i < p.width; i += maxStep) {
			for (int j = 0; j < p.height; j ++) {
				...
				for(int k = 0; k < maxStep - step; k++)
				{
					p.pixels[p.width * j + i + k] = theColor;
				}
			}
		}
		p.updatePixels();
